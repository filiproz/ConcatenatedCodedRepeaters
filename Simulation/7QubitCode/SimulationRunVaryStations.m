...
filename = sprintf('Data7QubitCode_sigGKP%d_eta%d.mat', round(100*sigGKP), round(100*eta_0));
...
Data7QubitCode = ones(40,40,3);

for NMultiQubit = MultiQubitStart:MultiQubitStationsRange
    NMultiQubit
    %For one multi-qubit repeater per 10 km we can calculate the optimal
    %rescaling coefficeints only up to 34 GKP repeaters (35 all repeaters)
    %per 10km, see the article Appendix C for details.
    if NMultiQubit == 1
        NallEnd = 35;
    else
        NallEnd = AllStationsRange;
    end
    for Nall = NMultiQubit:NMultiQubit:NallEnd
        ...
        %Run the simulation
        [psucc,Zerr,Xerr] = Code7Qubit(1, gamma, sigGKP, Nall/NMultiQubit, N);
        %
        %Estimate the error
